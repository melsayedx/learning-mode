...
leaves *confident* and wrong. This file is the host-neutral procedure for grounding
load-bearing factual claims about a specific technology.

Use the same method whether the active assistant verifies inline or delegates a precise
claim to a read-only verifier supported by the current host.
...
- **Conceptual / theoretical claim that requires verification** ("why does X exist", "what do most systems do here") → top-tier academic material: graduate and upper-undergraduate course materials from leading universities, plus the canonical texts of the field. Treat the named courses and books below as *examples of the bar*, not a closed list — use whichever top-tier source actually covers the topic. By domain: databases (CMU 15-445/645, Berkeley CS186), distributed systems (MIT 6.824), operating systems (OSTEP, MIT 6.S081), algorithms and data structures (MIT 6.006, Stanford CS161, CLRS, Skiena), networking (Stanford CS144), and the equivalents for compilers, architecture, and the rest. The defining property is the one a strong course has: it teaches the general model and the *reason* for it, not one product's API. Use these to establish the **general model first**.
- **Library / framework API specific** ("what does method M do", "what's the default", "is this deprecated") → **Context7**, version-pinned. Prefer the `ctx7` CLI when it is installed: resolve the name to an ID (`ctx7 library <name>`), then fetch docs (`ctx7 docs <library-id> <query>`). If the CLI is unavailable, use a connected Context7 tool. Fall back, in order, to the library's official documentation site, then its source and tests.
...
- **Secondary sources** never settle an API, protocol, or theoretical claim: independent blogs, Stack Overflow, tutorials, forum answers, and AI-generated summaries only help find a primary source. A team's own engineering write-up or postmortem can settle what that team did, but not establish a universal standard.
...
**Separate observed behavior from contractual behavior.** Direct evidence can expose
details that prose documentation omits, while a specification or contract establishes
...
- Evidence and documentation for the user's installed version beat documentation for a
  different or latest version.
...
Every claim selected for verification carries one of these labels. In live conversation,
show labels when confidence or provenance matters; do not decorate every stable sentence.
Persist labels for load-bearing claims in an enabled journal.
...
- **Teaching device** — an analogy, mental model, or example the assistant designed. Flag it as non-authoritative and state where it stops matching reality when that boundary matters.
...
When a lesson uses a naive-vs-production contrast, ground that contrast in verified fact
...
## 8. Optional verifier return contract

When the current host supports delegation, give the verifier the precise claim and this
method. Return a compact, citable result rather than raw documentation.
...
If the verdict is `unconfirmed`, the verifier additionally returns what it searched and
what specific source or experiment would settle it, so the active assistant can decide
whether to dig further or flag it to the learner.
...
6. **State the claim with a label and citation** when provenance matters. If journaling is enabled, persist load-bearing evidence there.
